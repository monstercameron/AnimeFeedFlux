...
var esSettingsMessages = gwci18n.NamespaceCatalog{
	...
	"settings.provider.providers.title": {Text: "Proveedores"},
	"settings.provider.providers.help":  {Text: "La generación usa un proveedor a la vez. Añade cualquier endpoint compatible con OpenAI — un servidor de modelos local, una pasarela, un revendedor — y cambia con un clic. Las claves se configuran aquí y se guardan cifradas en el servidor."},

	"settings.provider.builtin.help":       {Text: "OpenAI en su propia dirección. Pega su clave de API abajo: se guarda cifrada en el servidor."},
	"settings.provider.builtin.keyFromEnv": {Text: "Usando la clave del entorno del servidor (solo para desarrollo): pega una clave abajo para guardarla correctamente"},

	"settings.provider.active.badge": {Text: "Activo"},
	"settings.provider.active.use":   {Text: "Usar este proveedor"},

	"settings.provider.profile.namePlaceholder": {Text: "OpenRouter"},
	"settings.provider.profile.baseUrlHelp":     {Text: "Déjalo vacío para usar la dirección por defecto del proveedor."},
	"settings.provider.profile.removeLabel":     {Text: "Eliminar"},
	"settings.provider.profile.addHelp":         {Text: "Cualquier cosa que hable la API de OpenAI: OpenRouter, Azure, un llama.cpp u Ollama local."},

	"settings.provider.key.label":             {Text: "Clave de API"},
	"settings.provider.key.placeholder":       {Text: "Pega una clave para guardarla"},
	"settings.provider.key.placeholderStored": {Text: "•••••••• guardada — pega una clave nueva para sustituirla"},
	"settings.provider.key.stored":            {Text: "Clave guardada en el servidor, cifrada"},
	"settings.provider.key.fromEnv":           {Text: "Clave leída de {arg1}"},
	"settings.provider.key.none":              {Text: "Todavía sin clave"},
	"settings.provider.key.willStore":         {Text: "La clave se guardará, cifrada, al guardar"},
	"settings.provider.key.willClear":         {Text: "La clave guardada se eliminará al guardar"},
	"settings.provider.key.clear":             {Text: "Eliminar la clave guardada"},
	"settings.provider.key.undoClear":         {Text: "Conservar la clave guardada"},
	"settings.provider.key.help":              {Text: "Se guarda cifrada con la clave secreta del servidor y no se vuelve a mostrar: solo se puede sustituir o eliminar."},

	"settings.provider.envVar.label": {Text: "Variable de entorno de la clave (opcional)"},
	"settings.provider.envVar.help":  {Text: "Se usa cuando no hay una clave guardada aquí. Una clave guardada tiene prioridad sobre la variable."},

	"settings.provider.backend.label":      {Text: "Protocolo"},
	"settings.provider.backend.help":       {Text: "Qué dialecto de API habla el proveedor activo. La tarjeta del proveedor elige la dirección; esto elige el protocolo."},
	"settings.provider.backend.openai":     {Text: "OpenAI (por defecto)"},
	...
	"settings.provider.activeProvider.help":        {Text: "Qué backend se llama: openai, anthropic, openrouter, cerebras, deepseek, qwen o zai. Déjalo vacío para openai. Esto elige el protocolo; el perfil de abajo elige la dirección."},
	...
	"settings.provider.priceTable.col.in":         {Text: "$ de entrada por 1M tokens"},
	"settings.provider.priceTable.col.out":        {Text: "$ de salida por 1M tokens"},
	"settings.provider.priceTable.col.remove":     {Text: "Eliminar"},
	"settings.provider.priceTable.removeLabel":    {Text: "Eliminar"},
	"settings.provider.priceTable.removeRow":      {Text: "Eliminar la tarifa de {arg1}"},
